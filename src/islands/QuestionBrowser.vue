  ...
  <div class="min-h-dvh bg-bg text-text flex flex-col">
    ...
    <header class="border-b border-border bg-surface">
      <div class="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <h1 class="font-(--font-display) text-2xl text-gold">BANCO DE PREGUNTAS</h1>
        <div class="text-muted">{{ totalQuestions }} preguntas</div>
      </div>
    </header>
    ...
    <main class="flex-1 flex flex-col max-w-4xl mx-auto w-full px-4 py-8">
      ...
      <div class="flex items-center justify-between mb-8 gap-4">
        <button @click="previous" :disabled="currentIndex === 0"
          class="px-6 py-3 bg-surface border border-border rounded-lg font-semibold transition-all hover:bg-surface2 disabled:opacity-30 disabled:cursor-not-allowed">
          ← Anterior
        </button>

        <div class="flex flex-col items-center gap-2">
          <div class="text-sm text-muted">Pregunta</div>
          <input type="number" :value="currentIndex + 1"
            @input="(e) => goToQuestion(parseInt((e.target as HTMLInputElement).value) - 1)" min="1"
            :max="totalQuestions"
            class="w-20 px-3 py-2 bg-surface border border-border rounded-lg text-center font-mono text-lg focus:outline-none focus:ring-2 focus:ring-gold" />
          <div class="text-xs text-muted">de {{ totalQuestions }}</div>
        </div>

        <button @click="next" :disabled="currentIndex === totalQuestions - 1"
          class="px-6 py-3 bg-surface border border-border rounded-lg font-semibold transition-all hover:bg-surface2 disabled:opacity-30 disabled:cursor-not-allowed">
          Siguiente →
        </button>
      </div>
      ...
      <!-- Question Card -->
      <div class="bg-surface border border-border rounded-xl p-6 space-y-6 flex-1">
        <!-- Question Header -->
        <div class="flex items-start justify-between gap-4 flex-wrap">
          <div class="flex items-center gap-3">
            <span :class="getDifficultyColor(currentQuestion.difficulty)"
              class="px-3 py-1 rounded-full text-xs font-semibold border">
              {{ getDifficultyLabel(currentQuestion.difficulty) }} ({{ currentQuestion.difficulty }}%)
            </span>
            <span class="text-muted text-sm">ID: {{ currentQuestion.id }}</span>
            <span class="text-muted text-sm">⏱️ {{ currentQuestion.timeLimit }}s</span>
          </div>
          <div v-if="currentQuestion.image" class="text-xs `text-gold flex items-center gap-1">
            🖼️ Con imagen
          </div>
        </div>
        ...
        <div v-if="currentQuestion.image"
          class="rounded-xl overflow-hidden bg-bg border border-border flex items-center justify-center p-4">
          <img :src="currentQuestion.image.src" :alt="currentQuestion.image.alt"
            class="max-w-full max-h-80 object-contain" loading="eager" />
        </div>
        ...
        <div v-if="answerType === 'multiple-choice'" class="space-y-3">
          ...
          <button v-for="(option, index) in currentQuestion.options" :key="index" @click="selectAnswer(index)"
            :disabled="showResult" :class="[
              'w-full p-4 rounded-lg border-2 transition-all text-left',
              !showResult && 'hover:bg-surface2 hover:border-gold cursor-pointer',
              ...
              !showResult && 'bg-surface2 border-border',
              showResult && selectedAnswer !== index && index !== currentQuestion.correctIndex && 'bg-surface2 border-border opacity-50'
            ]">
            <div class="flex items-start gap-3">
              <span class="font-bold">{{ String.fromCharCode(65 + index) }}.</span>
              <span class="flex-1">{{ option }}</span>
              <span v-if="showResult && index === currentQuestion.correctIndex"
                class="text-green-400 font-semibold">✓</span>
              <span v-if="showResult && selectedAnswer === index && !isCorrect"
                class="text-red-400 font-semibold">✗</span>
            </div>
          </button>
        </div>
        ...
        <div v-if="answerType === 'text'" class="space-y-4">
          ...
          <div v-if="!showResult" class="flex gap-2">
            <input v-model="textAnswer" type="text" placeholder="Escribe aquí tu respuesta..."
              @keydown.enter="submitTextAnswer"
              class="flex-1 px-4 py-3 bg-surface2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-gold text-text" />
            <button @click="submitTextAnswer" :disabled="!textAnswer.trim()"
              class="px-6 py-3 bg-gold text-bg rounded-lg font-semibold transition-all hover:opacity-90 disabled:opacity-30 disabled:cursor-not-allowed">
              Enviar
            </button>
          </div>
        ...
        <div v-if="showResult && isCorrect"
          ...
          <p class="text-text leading-relaxed">{{ currentQuestion.explanation }}</p>
        </div>
        ...
        <div v-if="showResult && !isCorrect"
          ...
          <p class="text-muted">
            La respuesta correcta es: <span class="text-green-400 font-semibold">{{
              currentQuestion.options[currentQuestion.correctIndex] }}</span>
          </p>
        </div>
      </div>
    </main>
